...
Planteamos la hipótesis de que una política de terapia entrenada en \emph{self-play} adversarial contra un tumor que también aprende puede extender el tiempo de manejo simulado ---definido como el número de días en que el tumor permanece simultáneamente controlado en carga y mayoritariamente tratable--- por encima tanto de la dosis máxima tolerada como de la heurística de Gatenby. Asimismo, planteamos que el mecanismo descubierto puede corresponder a una dosificación intermitente que preserva la competencia de las células sensibles, y evaluamos empíricamente si el crítico centralizado mejora la fiabilidad del entrenamiento frente al aprendizaje independiente.
...
La evaluación tiene un peso equivalente. La métrica de tiempo a falla combinado está alineada con el objetivo de control y es auditable: evita premiar únicamente la reducción agresiva del tumor ---que minimiza la carga a corto plazo pero puede acelerar la resistencia--- y registra también la composición resistente. El beneficio esperado es metodológico: una política adaptativa que retrase la dominancia resistente puede generar hipótesis para el diseño de cronogramas de temozolomida, pero la transferencia a otros tumores requiere nueva calibración y validación.
...
\begin{enumerate}
  ...
  \item \textbf{Política adaptativa descubierta y su mecanismo.} Evaluamos una política de dosificación que, en la corrida calibrada nominal, extiende el tiempo a falla por encima de la dosis máxima tolerada y de la heurística adaptativa de Gatenby. Además, examinamos si su dosificación intermitente preserva una población sensible competidora; esta interpretación es mecanística dentro del simulador y no una demostración clínica.

  \item \textbf{Métrica de evaluación auditable.} Formalizamos una métrica de tiempo a falla combinado, que mide los días en que el tumor permanece controlado en carga \emph{y} mayoritariamente tratable, reportando el modo de falla. Esta métrica evita que una métrica de carga aislada oculte la mayoría resistente y se acompaña de un protocolo de validación multi-semilla con reporte por tasa de éxito y contraste estadístico exploratorio.
