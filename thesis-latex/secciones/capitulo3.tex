...
La calibración combina estadísticas de respuesta al fármaco con supuestos del simulador. Los datos determinan una escala de potencia relativa; el techo de muerte y las tasas de crecimiento no se estiman mediante un ajuste longitudinal en esta cohorte. La atribución de sus valores concretos a literatura requiere fuentes cuantitativas específicas.
...
Se promedia \texttt{LN\_IC50} por línea y se calculan los percentiles 10 y 90 de las 24 líneas con expresión completa. Las potencias relativas se definen como $\mathrm{IC50}_S=\exp(q_{10}-m)$ e $\mathrm{IC50}_R=\exp(q_{90}-m)$, donde $m$ es la mediana de \texttt{LN\_IC50}. Son magnitudes adimensionales y su razón es aproximadamente 9,1. Representar con esos extremos las poblaciones S/R es una hipótesis del simulador: los datos describen diferencias entre líneas, no subpoblaciones medidas dentro de un mismo tumor. El calibrador utiliza identificadores, nombre del fármaco, \texttt{LN\_IC50} y AUC; no utiliza las columnas de expresión para personalizar parámetros.

\subsection{Procedencia de los parámetros}

El techo de muerte se fija mediante $\delta_S^{\max}=\kappa\alpha_S$, con $\kappa=2{,}0$. Para resistentes se utiliza una razón de eficacia derivada de percentiles de AUC, limitada al intervalo $[0{,}5,1]$. Tanto $\kappa$ como el piso 0,5 son elecciones de diseño cuya fuente cuantitativa específica queda pendiente de verificar. La condición $\alpha_S-\delta_S^{\max}<0$ comprueba el límite de saturación de la función de muerte; no demuestra por sí sola control a una dosis finita. La Tabla~\ref{tab:procedencia} distingue estadísticas de datos, supuestos y normalización.

\begin{table}[H]
\centering
\caption{Procedencia de los parámetros. DATO: estadística de GDSC2 con asignación S/R asumida; SUP: supuesto cuyo valor cuantitativo requiere justificación adicional; NORM: normalización.}
\label{tab:procedencia}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{l|l|c|l}
\textbf{Parámetro} & \textbf{Símbolo} & \textbf{Valor} & \textbf{Procedencia} \\
\hline
Crecimiento sensibles      & $\alpha_S$        & 0.15  & SUP \\
Crecimiento resistentes    & $\alpha_R$        & 0.10  & SUP (costo de resistencia) \\
Capacidad de carga         & $K$               & 1.0   & NORM \\
Eliminación del fármaco    & $\lambda_c$       & 0.40  & SUP \\
Transición basal           & $\phi_{\text{base}}$ & 0.005 & SUP \\
IC50 sensibles             & $\mathrm{IC50}_S$ & 0.36  & DATO (GDSC2) \\
IC50 resistentes           & $\mathrm{IC50}_R$ & 3.27  & DATO (GDSC2) \\
Coeficiente de Hill        & $h$               & 2.0   & SUP \\
Muerte máx.\ sensibles     & $\delta_S^{\max}$ & 0.30  & SUP ($\kappa\,\alpha_S$) \\
Muerte máx.\ resistentes   & $\delta_R^{\max}$ & 0.15  & DATO + SUP (piso 0,5) \\
\end{tabular}
\end{table}

Esta parametrización permite reproducir un escenario computacional. Su conexión con dosis administradas y tiempos clínicos exige validar las escalas de concentración y tiempo, además de la dinámica longitudinal. Los días reportados son días del simulador bajo los parámetros especificados.

\section{Formulación como juego adversarial multiagente}
\label{sec:juego}

El problema de decisión se formula como un entorno multiagente paralelo siguiendo la interfaz \texttt{ParallelEnv} de PettingZoo~\cite{Terry2021}, con dos agentes que actúan de manera simultánea sobre el sistema~\eqref{eq:ode-S}--\eqref{eq:ode-c}. El estado dinámico real es tridimensional, $s=(S,R,c)$. La calibración vigente es agregada y compartida entre episodios; la expresión génica no condiciona actualmente los parámetros iniciales ni las políticas.
...
El desenlace se mide con el \textbf{tiempo a falla combinado} (TTP-combinado): el número de días hasta que el tumor deja de estar controlado~\eqref{eq:controlado} \emph{o} deja de ser tratable~\eqref{eq:tratable}, lo que ocurra primero, reportando además el \emph{motivo} de falla (progresión en carga, mayoría resistente o supervivencia al horizonte). Esta métrica está alineada con el objetivo de control y evita un artefacto sutil: una métrica basada únicamente en la resistencia asignaría falsamente el valor del horizonte a un episodio que en realidad falló por progresión de carga, inflando los resultados. No constituye un endpoint clínico validado; toda evaluación reporta el modo de falla para permitir su auditoría.

\subsection{Líneas base y validación}

La política aprendida se compara contra tres referencias deterministas evaluadas sobre el mismo simulador y bajo la misma métrica: la ausencia de tratamiento ($u \equiv 0$), la dosis máxima tolerada ($u \equiv u_{\max}$), y la terapia adaptativa heurística de Gatenby (dosificación de encendido/apagado según umbrales de carga)~\cite{Gallaher2017,Stankova2019}. La validación se realiza sobre múltiples semillas independientes. Cuando se observa dispersión o posible bimodalidad en el \emph{self-play}, los resultados se reportan por mediana, rango y tasa de éxito, en lugar de depender de un promedio; los contrastes estadísticos se interpretan de acuerdo con el tamaño muestral y el diseño pareado.

\subsection{Estudios de ablación y sensibilidad}

Dos estudios complementan la validación principal. El \textbf{estudio de ablación} compara MAPPO (crítico centralizado) frente a IPPO (crítico local) sobre las mismas semillas, aislando la contribución de CTDE. El \textbf{análisis de sensibilidad} varía, una a la vez, ocho parámetros de dinámica y fármaco alrededor de su valor nominal, y añade perturbación conjunta, ruido relativo de observación y sensibilidad de los umbrales del endpoint. El objetivo es verificar en qué condiciones se conserva el ordenamiento entre estrategias y localizar sus fragilidades, no asumir robustez por defecto. Los resultados se presentan en el Capítulo~\ref{cap:resultados} y en los reportes reproducibles del código.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{images/pipeline_diagram.png}
...
2. Calibración & Derivar IC50 relativa de datos; declarar Hill y tasas como supuestos. & Fija la \emph{potencia} relativa y documenta los supuestos del simulador. \\
